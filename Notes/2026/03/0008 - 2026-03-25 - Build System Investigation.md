...
<!-- TOPIC: Obsidian Strip Script, Build Pipeline Separation, and Template Wrapping -->

**Date:** 2026-03-25
**Status:** Completed

**Completed:** 2026-03-25T12:41:22.641Z
...
- **No git commits made** — all changes to `make.sh`, `templates/parser.pl`, `templates/strip_obsidian.pl` (new), `template.md`, `include.md`, and new MOC files remain uncommitted
- **Old `- -.md` MOC files** — user confirmed they should be deleted but deletion hasn't been executed yet
- **YAML `links:` frontmatter structure** for content files still needs finalization — user began explaining the desired link approach but the conversation was interrupted before the full design was captured
- **Full build not yet verified** — the strip pipeline components test correctly in isolation but a complete end-to-end build (through pdflatex) hasn't been run since the `strip_obsidian.pl` refactor
